...
一次定位完成后，完整移走模块目录并恢复安装前的 `[network_mitm]` 设置；尤其删除 `enable_account_link_diagnostic`。不要保留普通 `ssl` 的诊断版作为常驻模块。仅想关掉 NIM fallback 时，可设 enable_device_cert_fallback=0 后完整重启；它仍不会重新扫描全系统。
